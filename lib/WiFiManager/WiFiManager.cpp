...
void WiFiManager::addParameter(WiFiManagerParameter *p) {
  if(_paramsCount + 1 > WIFI_MANAGER_MAX_PARAMS)
  {
	...
	DEBUG_WM("WIFI_MANAGER_MAX_PARAMS exceeded");
...
void WiFiManager::handleRoot() {
  ...
  String page = FPSTR(HTTP_WIFIMAN_HEAD);
  page.replace("{v}", "Options");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_WIFIMAN_HEAD_END);
  page += "<h1>";
  page += _apName;
  page += "</h1>";
...
void WiFiManager::handleWifiSave() {
  ...
  _mqtt_username = server->arg("u").c_str();
  _mqtt_password = server->arg("w").c_str();
  //parameters
  for (int i = 0; i < _paramsCount; i++) {
    if (_params[i] == NULL) {
      break;
